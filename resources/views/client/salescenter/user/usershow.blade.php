						  ...
						  <div class="col-xs-12 col-sm-12 col-md-12 sales_tablebx">
								<div class="table-responsive">
									...
									<div class="btnintable bottom_btns">
										<div class="btn-group">
                                        <?php edit_btn_on_view_info_page( route('client.salescenter.user.edit',['client_id' => $client_id, 'salescenter_id'=>$user->salescenter_id, 'userid' =>$user->id  ])) ?>

                                              <?php if($user->status=='active'){ ?>
                                                    <a
                                                    class="deactivate-salescenteruser btn btn-red"
                                                    ...
                                                    data-uid="{{ $user->id }}"
                                                    id="delete-salescenteruser-{{ $user->id }}"
                                                    data-salescenteruser="{{ $user->first_name }} {{ $user->last_name }}"
                                                     >Deactivate<span class="del"><?php
                                                     echo getimage('images/activate.png') ?></span> </a>
                                                        <?php } else {?>
                                                    <a class="activate-salescenteruser  btn btn-green"
                                                        href="javascript:void(0)"
                                                        data-toggle="tooltip"
                                                        data-placement="top" data-container="body"
                                                        title=""
                                                        ...
                                                        id="delete-salescenteruser-{{ $user->id }}"
                                                        data-salescenteruser="{{ $user->first_name }} {{ $user->last_name }}" >
                                                        Activate<span class="add"><?php echo getimage('images/deactivate.png') ?></span></a>
                                               <?php }?>

...
@section('content')
 ...
 <div class="tpv-contbx">
			<div class="container">
                <div class="tpvbtn">
                    <div class="col-xs-12 col-sm-12 col-md-12"> <h2>Lead Fields Mapping</h2></div>
                    <form class="form-horizontal" method="POST" action="{{ route('client.importleaddata',[$client_id]) }}">
                            {{ csrf_field() }}
                                ...
                                <thead>
                                    ...
                                    @if($i==0)
                                      ...
                                      <?php  $j++;   if($j % 2 == 0){
                                        ...
                                        ?>
                                        <tr>
                                            <td class="{{ $first_last_td_class}}">{{ $value }}</td>
                                            <td class="{{ $second_and_middle_td_class}}">{{ $csv_data[1][$key]}}</td>
                                            <td class="{{ $first_last_td_class}}">
                                                <select name="fields[{{ $value }}]" class="selectsearch" >
                                                    <option value=''>Select</option>
                                                    @foreach ($database_fields as $db_field)
                                                        <option value="{{ $db_field }}"
                                                            @if (strtolower($value) === strtolower($db_field)) selected @endif>{{ $db_field }}</option>
                                                    @endforeach
                                                </select></td>
                                            </tr>
                                        @endforeach

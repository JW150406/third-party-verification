...
class EnrollmentReportController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        Log::info("In index method of EnrollmentReportController");
        ...
        if($request->ajax()) {
            ...
            ->editColumn('TPVDate', function($enrollment)use($timeZone){
                $date = '';
                if(!empty($enrollment->TPVDate) && ($enrollment->Status != config('constants.VERIFICATION_STATUS_CHART.'.ucfirst('pending'))) && ($enrollment->Status != config('constants.VERIFICATION_STATUS_CHART.'.ucfirst('expired'))) && (($enrollment->Method != 'Self Verification(Email)') && ($enrollment->Method != 'Self Verification(SMS'))) {
                    $date= Carbon::parse($enrollment->TPVDate)->setTimezone($timeZone)->format(getDateFormat().' '.getTimeFormat());
                }
                return $date;
            })
            ->editColumn('TPVAgent', function($enrollment)use($timeZone){
                $name = '';
                if(!empty($enrollment->TPVAgent) && ($enrollment->Status != config('constants.VERIFICATION_STATUS_CHART.'.ucfirst('pending'))) && ($enrollment->Status != config('constants.VERIFICATION_STATUS_CHART.'.ucfirst('expired'))) && (($enrollment->Method != 'Self Verification(Email)') && ($enrollment->Method != 'Self Verification(SMS'))) {
                   $name = $enrollment->TPVAgent;
                }
                return $name;
            })
            ->rawColumns(['Programs','Esignature'])
            ->make(true);
        }
        $clients = getAllClients();
        $brands = (new Brandcontacts)->getBrandsByClient($clientId);
        $results = '';
        return view('reports.enrollment_report.index', compact('results', 'clients', 'salesCenter','brands'));
    }
    ...
    {
        Log::info("In exportReport method of EnrollmentReportController");
